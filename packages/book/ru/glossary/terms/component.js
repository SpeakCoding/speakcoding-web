...
export const Definition = () => (
    <>
        <em>{name}</em> - элемент описания класса, то из чего будут состоять все конкретные
        экземпляры класса. Прим.: все комментарии будут состоять из имени пользователя и текста
        самого комментария. Текст и имя пользователя - составляющие класса Комментарий.
    </>
);
