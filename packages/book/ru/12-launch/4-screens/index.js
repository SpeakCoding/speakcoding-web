    ...
    <Section>
        <Section.Block>
            <Section.Main hlactive narrow>
                ...
                <p>
                    После того, как пользователь залогинился, completion функция откроет корневой
                    экран приложения (в нашем случае - экран с пятью табами), для этого будут
                ...
                <p>
                    Мы уже поговорили о том, что каждый таб - это экземпляр класса
                    ...
                    ViewController в NavigationController. Остается для каждого таба задать
                    tabBarItem (экземпляр класса UITabBarItem), в который мы передаем две картинки
                    (для состояния, когда этот таб активен и нет), а также задать идентификационный
                    tag.
                </p>
            </Section.Main>
        </Section.Block>
