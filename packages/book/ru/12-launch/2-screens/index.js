        ...
        <Section>
            <Section.Block>
                <Section.Main hlactive narrow>
                    ...
                    <p>
                        ...
                        с 29 строки. В ней мы видим описание функции application, c двумя
                        стандартными вводными, определенными в классе UIApplication, который мы
                        наследуем:{' '}
                        <HL color='aquamarine'>экземпляр application класса UIApplication</HL>, а
                        также функцию didFinishLaunchingWithOptions с набором возможных опций для
                        запуска - launchOptions.
                    </p>
                    <Pre>{code1}</Pre>
                    <p>
                        Первым шагом эта функция вызовет User.
                        <HL color='light-sky-blue'>initCurrentUser()</HL>, которая определена в
                        классе User (соответственно, в файле User.swift). Она проверит, сохранен ли{' '}
                    ...
                    <p>
                        ...
                        <HL color='steel-blue'>rootViewController</HL>, заменяя на тот
                        ViewController, который нам будет необходим.
                    </p>
                    <p>
                        Функция <HL color='sandy-brown'>showLoginView</HL>() описывает все действия,
                        которые нужно совершить для отображения в нашем окне экрана LogIn. В
                        частности, она включает шаг, который кладет в составляющую self.window.
                        <HL color='steel-blue'>rootViewController</HL> контроллер LoginVIew.
                    </p>
                    <p>
                        Функция setupUI() создает экземпляр стандартного класса UITabBarController.
                        Как вы помните, он отвечает за отображение нижнего меню “табов” нашего
                        приложения (вкладки home, item search (поиск пользователей), userprofile,
                        likes и newpost) и позволяет пользователю переключаться между основными
                        экранами приложения.
                    </p>
                    <p>
                        Дальше мы подробнее поговорим о шагах этих двух функций и зачем они нужны,
                        но заметьте, что{' '}
                        <b>
                            функция application описывает основное назначение файла и класса
                            AppDelegate
                        </b>
                        :
                    </p>
                    <ul>
                        ...
                        <li>
                            и какие контроллеры нужно запустить для этого отображения и
                            взаимодействия.
                        </li>
                    </ul>
                </Section.Main>
            ...
            <Section.Block>
                <Section.Main hlactive narrow>
                    ...
                    <p>
                        Итак, вы можете думать о приложении как о некотором экране, в описании
                        которого мы можем задать, какой ViewController в нем показывать.
                    </p>
                    <p>
                        В предыдущей функции мы вызвали экран LoginView через функцию{' '}
                        <HL color='aquamarine'>showLoginView</HL>. Ниже в коде следует ее
                        определение. Как видите, в ней мы создаем экземпляр{' '}
                        <HL color='light-sky-blue'>LoginViewController</HL>. Создаем по умолчанию
                        без email и передаем completion функцию, которая выполнится, когда человек
                        успешно войдет в систему.
                    </p>
                    <Pre>{code3}</Pre>
                    <p>
                    ...
                    <p>
                        ...
                        , и его{' '}
                        <HL color='turquoise'>верхнее меню меню будет по умолчанию спрятано</HL>.
                    </p>
                </Section.Main>
            </Section.Block>
            ...
            <Section.Block>
                <Section.Main hlactive>
                    ...
                    <p>
                        Что касается функции setupUI, в ней мы создаем экземпляр UItabBarController
                        и пять кнопок (tab’ов) нижнего меню основного экрана. Мы сохраняем эти
                        вкладки как набор в составляющей “viewcontroller” tabbarcontrollers.
                        Экземпляр этого класса будет помещен в self.tabBarController, и затем в
                        self.window.rootViewController нашего окна. Это означает, что пользователь
                        увидит в окне соответствующий экран (корневой экран с пятью вкладками).
                    </p>
                    <p>
                        ...
                        экземпляр класса одного из основных наших ViewControllers. Например, для
                        первой вкладки - это FeedViewController. Можно сказать, что мы как бы
                        “обернули” ViewController в NavigationController. Остается для каждой
                        вкладки задать tabBarItem (экземпляр класса UITabBarItem), в которую мы
                        передаем две картинки (для состояния, когда этот tab активен и нет), а также
                        задать идентификационный tag.
                    </p>
                </Section.Main>
