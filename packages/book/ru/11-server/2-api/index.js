import React from 'react';
import { Pre, Section } from '@sc/ui';
import { Assignment } from '../../../components';
import Term from '../../glossary/Term';

const code1 = `resources :posts, only: [:create, :show, :index, :update, :destroy]`;

const code2 = `
member do
    post :like, :unlike, :save, :unsave
    get :likers
end`;

const code3 = `
collection do
    get :saved, :tagged
end`;

export default () => (
    <>
        <Section>
            <Section.Main narrow>
                <h2>API</h2>
                <p>
                    Мы обсудили создание баз данных и описание необходимых классов. Теперь нам нужно
                    реализовать непосредственно обработку приходящих с клиента запросов и подготовку
                    ответов для них.
                </p>
                <p>Для этого на сервере нужно описать функцию, которая бы отвечала:</p>
                <ul>
                    <li>за то, чтобы принимать запросы,</li>
                    <li>читать и разбирать URL на переданные параметры,</li>
                    <li>
                        создавать экземпляры соответствующих контроллеров и запускать их функции,
                        соответствующие действию, которое необходимо произвести (создать, обновить,
                        и т.д.).
                    </li>
                </ul>
                <p>
                    Как мы упоминали, так как на сервере используются стандартные библиотеки
                    фреймворка <Term id='ruby-on-rails'>Ruby On Rails</Term> (о нем мы поговорим
                    чуть позже), все эти действия могут произвести стандартные функции, в коде
                    достаточно только указать “пути” (на какие точные URL серверная программа должна
                    реагировать) и назвать сами действия.
                </p>
                <p>В файле config/routes.rb мы описываем все эти URL:</p>
                <Pre>{code1}</Pre>
                <p>
                    Вызвав функцию resources, мы сообщаем, что у нас есть посты и их можно
                    создавать, отображать, просматривать списком, обновлять и удалять.
                    Соответственно, подразумевается, что вы уже написали controller с именем
                    post_controller и что в нем есть необходимые функции, которые здесь перечислены.
                </p>
                <p>
                    Итак, серверная программа будет обрабатывать запросы по URL и отвечать на них
                    соответствующим endpoint’ам объектами JSON.
                </p>
                <p>
                    Для нестандартных функций (таких как follow, о которой мы говорили выше) мы
                    должны будем указать, к чему они применимы: функция <b>member</b> будет
                    означать, что описанные действия будут применены к конкретному экземпляру:
                </p>
                <Pre>{code2}</Pre>
                <p>
                    функция <b>collection</b> - ко всему множеству объектов.
                </p>
                <Pre>{code3}</Pre>
                <p>
                    Обязательно убедитесь, что все классы (users/posts/ etc) и их контроллеры
                    называются так, как вы описали в функции (save/unsave).
                </p>
            </Section.Main>
        </Section>

        <Assignment id='ru/29' />
    </>
);
