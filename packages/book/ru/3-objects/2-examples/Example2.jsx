    ...
    <Section.Block>
        <Section.Main>
            <p>
                ...
                , <b>создающая</b> конкретный объект (<Term id='instance'>экземпляр</Term>) класса
                должна быть названа определенным образом. Такое название является жестко заданным
                для каждого языка. В некоторых языках она повторяет название класса, а в некоторых,
                таких как Python, называется __init__. Это название всегда можно найти, набрав в
                поисковике конкретный язык.
            </p>
            <p>
                Теперь, когда мы задали функцию-конструктор, мы можем создавать объекты (экземпляры)
                класса, вызывая ее. Во многих языках вызов этой функции сопровождается
                словом-индикатором{' '}
            ...
            <p>
                ...
                которая будет вызываться при нажатии кнопки Submit. Она возьмет данные из формы и
                выполнит последовательность шагов по созданию new User.
            </p>
            <Grid cols='36% 1fr'>
