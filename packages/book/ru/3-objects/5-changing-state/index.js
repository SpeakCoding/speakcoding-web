            ...
            <Section>
                <Section.Block>
                    <Section.Main>
                        <InterceptorView onEnter={onEnter} onExit={onExit}>
                            ...
                            <p>
                                Когда у нас появились <i>конкретные</i>{' '}
                                <Term id='instance'>объекты</Term> какого-то{' '}
                                <Term id='class'>класса</Term> и мы научились их сохранять, мы можем
                                менять их состояние. Например, когда мы создали аккаунт конкретного
                            ...
                            <p>
                                Давайте представим, что в Инстаграме есть форма, которая позволяет
                                изменить ваше имя пользователя. В этой форме есть поле ввода нового
                                имени пользователя newUsernameField, и кнопку Save. Форма хранит{' '}
                                <Term id='instance'>объект</Term> (экземпляр) вашего аккаунта (User)
                                в своей <Term id='component'>составляющей</Term> currentUser{' '}
                                <Term id='class'>класса</Term> User. И в ее{' '}
                                <Term id='class'>классе</Term>
                                ...
                                <Term id='component'>составляющую</Term> username вашего аккаунта
                                (объекта класса User).
                            </p>
                            <ul>
                                <li>
                                    Можно заменять <Term id='component'>составляющие</Term> или
                                    менять <Term id='property'>характеристики</Term> напрямую, через
                                    добавление соответствующего действия в последовательность шагов
                                    функции onClickSetButton.
                                </li>
                            </ul>
                        </InterceptorView>
                    </Section.Main>
                ...
                <Section.Block>
                    <Section.Main narrow>
                        ...
                        <p>
                            При описании действий, которые <Term id='function'>функция</Term>{' '}
                            выполняет, мы можем обращаться к{' '}
                            <Term id='component'>составляющим</Term> экземпляра напрямую. Т.к.{' '}
                            <Term id='instance'>экземпляр класса</Term> содержит все{' '}
                            <Term id='component'>составляющие</Term>, перечисленные в описании этого
                            класса, нам не нужно будет добавлять их как{' '}
                            <Term id='input'>вводные</Term>. Давайте разберем на примере ниже.
                        </p>
                        <p>
                            Например, мы хотим отредактировать текст поста. Для этого в описании{' '}
                            <Term id='class'>класса</Term> Post мы создадим функцию editCaption().
                            Для выполнения этой <Term id='function'>функции</Term> нам понадобится
                            обновленный текст поста, т.е. он будет являться ее{' '}
                            <Term id='input'>вводной</Term>, а сама{' '}
                            <Term id='function'>функция</Term> выглядит как последовательность
                            следующих действий:
                        </p>
                    </Section.Main>
                </Section.Block>
                ...
                <Section.Block>
                    <Section.Main narrow>
                        <p>
                            Как мы записали, Caption является составляющей класса Post. Поэтому,
                            когда мы создаем конкретный Post, у нас будет доступ ко всем
                            составляющим этого класса.
                        </p>
                        <p>
                            Получается, нам не нужно добавлять в код шаг указания конкретного текста
                            под этим постом. Поскольку у каждого Поста всегда есть текст, а функция
